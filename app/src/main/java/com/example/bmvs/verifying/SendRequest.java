...
/**
 * SendRequest class manages sending verification requests to multiple nodes
 * and handles receiving their confirmation responses.
 */
        ...
        receiveResponse.receiveData(data -> {
            try {
                ...
                if (Objects.equals(imHash, responseJson.getString("im_hash"))) {
                    ULog.add("Image(" + imHash.substring(0,5) + ") confirmation from "
                            + responseJson.getString("address"));
                } else {
